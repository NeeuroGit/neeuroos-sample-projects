...
4. Implement an initialization script that: [page:1]  
   - Injects the Developer Code.  
   - Registers callbacks for EEG/PPG/metrics data and connection/authentication events.  
...
- Copy or import the provided Unity package into the project (for example, via `Assets > Import Package > Custom Package` if a `.unitypackage` is provided). [page:1]  
- Verify that: [page:1]  
  - C# scripts for NeeuroOS are present under a dedicated folder.  
  - Native libraries/plugins for Android, iOS, and/or Windows are present under `Plugins` with proper platform settings.
...
- Create or choose a bootstrap scene (for example, `NeeuroBootstrap`).  
- Add the `NSB_BLE` and `NSB_EEG` prefabs into the scene hierarchy.  
- Ensure they are not destroyed on load if needed (for example, mark with `DontDestroyOnLoad` in a manager script) so data streaming can persist across scenes. [page:1]
...
1. Create a C# script, for example `NeeuroInitializer.cs`. [page:1]  
2. Attach it to an empty `GameObject` in the bootstrap scene. [page:1]  
3. Implement initialization logic that: [page:1]  
   - Passes the Developer Code into the NeeuroOS Unity API.  
   - Wires necessary callbacks for authentication status and result, and validity period.
...
public class NeeuroInitializer : MonoBehaviour
{
    ...
    {
        // Handle ongoing authentication status updates (for example, countdown validity).
    }
...
1. Create a manager script (for example, `SenzeBandManager.cs`) that interacts with `NSB_BLE`. [page:1]  
...
- Expose methods mapped to commands: [page:1]  
  - Switch to AC mode (impedance measurement) and back to DC mode.  
  - Start/stop calibration for accelerometer/orientation.
...
The agent should optionally display impedance values and channel status via callbacks analogous to `EEG_GetImpedance` and `EEG_ChannelStatus` in native docs. [page:1]
...
1. Implement methods for controlling streams: [page:1]
...
2. Register data callbacks for: [page:1]  
   ...
   - Accelerometer values, orientation, etc.
...
- Ensure:  
  ...
  - Required permissions (Bluetooth, location as required by OS version) are declared (may be handled by the plugin).  
...
- Deploy to device and validate connection and streaming.
...
- Build player, run on a PC with Bluetooth capability (or dongle) and test scanning/connection/data streaming.

---

## 10. Recommended Scene & Script Structure

For a clean integration automated by an AI agent: [page:1]

- `Scenes/`  
  - `NeeuroBootstrap.unity` – contains `NSB_BLE`, `NSB_EEG`, `NeeuroInitializer`, and `SenzeBandManager`.  

- `Scripts/Neeuro/`  
  - `NeeuroInitializer.cs` – handles Developer Code and SDK initialization plus authentication.  
...
- Optional UI scripts for device list, connect/disconnect buttons, and metric visualizations. [page:1]
