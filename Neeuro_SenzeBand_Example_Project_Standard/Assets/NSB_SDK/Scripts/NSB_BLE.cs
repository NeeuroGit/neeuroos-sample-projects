...
public class NSB_BLE : MonoBehaviour
{
	...
	*/
	/******************************************************************************/
	public void assignScanCallBack(stringStringReturnVoid scanCallBackIn)
	{
		BLEController.getInstance().assignScanCallBack(new EEGController.EEGDoubleStringCallBack(scanCallBackIn));
	}
	...
	*/
	...
	{
#if (UNITY_STANDALONE_WIN || UNITY_EDITOR) && NET_4_6 && !UNITY_EDITOR_OSX
		BLEController.getInstance().grabInputCommand(command);
#elif UNITY_ANDROID
		BLEController.getInstance().grabInputCommand(command);
#elif UNITY_IOS
		BLEController.getInstance().grabInputCommand(command);
	...
	*/
	...
	private void connectionFail(string addressAndErrorLog)
	{
		...
		if(address.Contains(">"))
			address = address.Substring(address.IndexOf(">") + 1);	//UUID has 36 characters
