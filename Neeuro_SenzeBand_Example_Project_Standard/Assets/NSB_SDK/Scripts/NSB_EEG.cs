...
#elif (UNITY_STANDALONE_WIN || UNITY_EDITOR) && NET_4_6 && !UNITY_EDITOR_OSX
...
public class NSB_EEG : MonoBehaviour
{
	...
	*/
	...
	\brief The assigned delegate will be called when the raw EEG signal is ready. The raw signal has is an array of 1000 ints.
		   Each int value ranges from -2048 to 2047. Every 1 unit represents 0.61 microvolt.
	\param callBackFunc
	The callBack function to execute
	*/
	/******************************************************************************/
	...
	/******************************************************************************/
	/*!
	\fn  void assignRawDataDelegateFloat(MyEEGHandler.btFloatArrayCallback callBackFunc)
	...
	*/
	/******************************************************************************/
	public void assignRawDataDelegateFloat(EEGController.btFloatArrayCallback callBackFunc)
	{
		EEGController.getInstance().assignRawDataDelegateFloat(callBackFunc);
	}

    /******************************************************************************/
    /*!
	\fn  void assignRawDataDelegate200ms(MyEEGHandler.btFloatArrayCallback callBackFunc)
	\brief The assigned delegate will be called when the raw EEG signal is ready. The raw signal has is an array of 200 floats.
		   The first 50 floats belong to one channel, next 50 floats belong to another channel and so on.
	\param callBackFunc
	The callBack function to execute
	*/
    /******************************************************************************/
    public void assignRawDataDelegate200ms(EEGController.btFloatArrayCallback callBackFunc)
    {
        EEGController.getInstance().assignRawDataDelegate200ms(callBackFunc);
    }
	...
	*/
	...
	public void assignFilteredDataDelegate(EEGController.btFloatArrayCallback callBackFunc)
	{
		EEGController.getInstance().assignFilteredDataDelegate(callBackFunc);
	}
	public void assignDirectionDelegate(EEGController.EEGStringCallBack callBackFunc)
	{
		EEGController.getInstance().assignDirectionDelegate(callBackFunc);
	}

	public void assignCalibrationParametersDelegate(EEGController.btFloatArrayCallback callBackFunc)
	{
		EEGController.getInstance().assignCalibrationParametersDelegate(callBackFunc);
	}

	public void assignSPO2AndHeartRateDelegate(EEGController.btIntArrayCallback callBackFunc)
	{
		EEGController.getInstance().assignSPO2AndHeartRateDelegate(callBackFunc);
	}

	/******************************************************************************/
	/*!
	\fn  void assignPPGDataDelegate(MyEEGHandler.btFloatArrayCallback callBackFunc)
	\brief The assigned delegate will be called when the raw PPG signal is ready. The raw signal has is an array of 1000 floats.
		   The first float is IR and the second float is RED.
	\param callBackFunc
	The callBack function to execute
	*/
	/******************************************************************************/
	public void assignPPGDataDelegate(EEGController.btIntArrayCallback callBackFunc)
	{
		EEGController.getInstance().assignPPGDataDelegate(callBackFunc);
	}

	/******************************************************************************/
	/*!
	\fn  void assignEEGImpedanceDelegate(MyEEGHandler.btFloatArrayCallback callBackFunc)
	\brief The assigned delegate will be called when the EEG impedance is ready.
	\param callBackFunc
	The callBack function to execute
	*/
	/******************************************************************************/
	public void assignEEGImpedanceDelegate(EEGController.btFloatArrayCallback callBackFunc)
	{
		Debug.Log("assignEEGImpedanceDelegate");
		EEGController.getInstance().assignEEGImpedanceDelegate(callBackFunc);
	}

    /******************************************************************************/
    /*!
	\fn  void assignCommandACKDelegate(EEGController.EEGStringCallBack callBackFunc)
	\brief The assigned delegate will be called whenever the NEEURO plugin confirms AC or DC leadoff change
	The callBack function to execute
	*/
    /******************************************************************************/
    public void assignCommandACKDelegate(EEGController.EEGStringCallBack callBackFunc)
    {
        EEGController.getInstance().assignCommandACKDelegate(callBackFunc);
    }
	...
	*/
	/******************************************************************************/
	public void assignSenzeBandVersionDelegate(EEGController.EEGStringCallBack callBackFunc)
	{
		EEGController.getInstance().assignSenzeBandVersionCallBack(callBackFunc);
	}
	...
	*/
	...
	public void unloadDelegates()
	{
        ...
        EEGController.getInstance().assignBatteryUpdateCallBack(null);
        EEGController.getInstance().assignDirectionDelegate(null);
        EEGController.getInstance().assignCalibrationParametersDelegate(null);
        EEGController.getInstance().assignSPO2AndHeartRateDelegate(null);
        EEGController.getInstance().assignEEGImpedanceDelegate(null);
        EEGController.getInstance().assignSenzeBandVersionCallBack(null);
        EEGController.getInstance().assignCommandACKDelegate(null);
    }
